...
function OrderItem({ oItem, setRefresh }: any) {
    ...
    function submitReview(review: any) {
        api.put('order/review', { ...order, review: review }).then(_ => {
    ...
    function confirmOrder() {
        api.put('order', { ...order, status: 'Received', receivedDate: new Date().toLocaleString() }).then(_ => {
    ...
    function cancelOrder() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                api.put('order', { ...order, status: 'Canceled' }).then(_ => {
